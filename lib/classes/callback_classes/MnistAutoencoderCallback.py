...
class MnistAutoencoderCallback(MyRecordingCallback):

    def __init__(self, master_config):
        super(MnistAutoencoderCallback, self).__init__(master_config)
    ...
    def _collect_comparisons(self):
        ...
        tensor_model = Model(inputs=self.model.inputs, outputs=self.model.callback_tensor_dict["latent_vector"])
        latent_vector_out = tensor_model.predict(x_test)
        print("Latent vector ou shape: {}".format(latent_vector_out.shape))
        ...
        for index_list in digit_list:
            for index in index_list:
                ...
                ax.set_xticks([])
                ax.set_yticks([])
                ax.imshow(np.zeros(shape=(84, 84)), cmap="gray")
                fig.add_subplot(ax)
